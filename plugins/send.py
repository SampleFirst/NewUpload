...
async def send_to_admin(bot, message):
    if message.reply_to_message:
        if message.from_user.id not in ADMINS:
            ...
            await forwarded_message.forward(chat_id=ADMINS[0])
            await message.reply_text(f"<b>Your message has been successfully sent to Admin by {sender_name}.</b>")
            return
        else:
...
async def forward_reply_to_user(bot, message):
    ...
    from_user = None
    sender_name = None
    from_user_id, user_first_name = extract_user(message)
    try:
    ...
    if forwarded_message:
        sender_name = forwarded_message.from_user.first_name
        await forwarded_message.forward(chat_id=from_user_id)
        await status_message.edit(f"<b>Your message has been successfully sent to {user_first_name} by {sender_name}.</b>")
    else:
        await status_message.edit("No message to forward.")
    return
